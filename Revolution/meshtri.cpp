...
void MeshTri::revolution(const std::vector<Vec3>& poly)
{
    ...
    float D = 1;
    for(int i=0;i<360-D;i+=D){
        for(int j=0;j<(int)poly.size();j++){
            Vec3 tmp = poly[j];
            tmp.x *= cos(i*2*M_PI/360);
            tmp.z = sin(i*2*M_PI/360) * poly[j].x;
            add_vertex(tmp);
        }
    }

    for(int i=0; i<(int)360/D -2 ;i++){
        for(int j=0; j<(int)poly.size()-1;j++){
            //i*poly.size()+j
            add_quad(i*poly.size()+j,i*poly.size()+j+1,(i+1)*poly.size()+j+1,(i+1)*poly.size()+j);
        }
    }
    ...
    for(int j=0; j<(int)poly.size()-1;j++){
        //i*poly.size()+j
        int i=(int)360/D -2;
        add_quad(i*poly.size()+j,i*poly.size()+j+1,j+1,j);
    }
